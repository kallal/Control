...
Private Function IsNotVCS(ByVal name As String) As Boolean
    If name <> "GITMaint" And _
      name <> "VCS_IE_Functions" Then
        IsNotVCS = True
    Else
        IsNotVCS = False
    End If
   ...
   ' bolMsg = true = show progress bar
    ...
    For Each qry In db.QueryDefs
       ...
       If Left$(qry.name, 1) <> "~" Then
          bolExportFlag = True
          If bolOnlyNew = True Then
             If dtLastExport > qry.LastUpdated Then
                bolExportFlag = False
             End If
          End If
       End If
...
skipquery:
    ...
    For Each obj_type In Split( _
        ...
        Select Case obj_type_name
           ...
           Case "modules": bolExportYes = bolEModule: Stop
       End Select
         ...
         For Each doc In db.Containers(obj_type_name).Documents
             ...
             If (Left$(doc.name, 1) <> "~") And _
                (IsNotVCS(doc.name) Or ArchiveMyself) Then
                 ...
                 End If
If doc.name = "Git" Then
   Stop
End If
                bolExportFlag = True
                If bolOnlyNew = True Then
                   If dtLastExport > doc.LastUpdated Then
                      bolExportFlag = False
                   End If
                End If
